...
class TaskListTest {
    ...
    void testAddToTaskList_todoCommand_success() throws Exception {
        ...
        String[] instruction = new String[] {"add", "todo read book"};
        Task task = taskList.createTask(instruction);
    ...
    void testAddToTaskList_deadlineCommand_success() throws Exception {
        TaskList taskList = new TaskList(new ArrayList<Task>());
        String[] instruction = new String[] {"add", "deadline submit report /by 2024-09-01T23:59:59"};
        Task task = taskList.createTask(instruction);
        taskList.addToTaskList(task);
        assertEquals(1, taskList.getTaskCount());
    ...
    void testAddToTaskList_eventCommand_success() throws Exception {
        TaskList taskList = new TaskList(new ArrayList<Task>());
        String[] instruction = new String[] {"add", "event school project meeting " +
    ...
    void addToTaskList_invalidDateFormat_dateTimeParseExceptionThrown() {
        ...
        assertThrows(DateTimeParseException.class, () -> {
            String[] instruction = new String[] {"add", "deadline submit report /by wrongFormat"};
            Task task = taskList.createTask(instruction);
            taskList.addToTaskList(task);
        });
    }
