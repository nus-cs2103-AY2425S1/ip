...
public class ParserTest {
    ...
    public void tokenize_success() {
        ...
        assertEquals(Map.of("CMD", "blah",  "dummy", "dummy value"),
                     Parser.tokenize("blah /dummy dummy value"));
        assertEquals(Map.of("CMD", "please",  "do", "this", "and", "that"),
                     Parser.tokenize("please /do this /and that"));

        // Tokenizing a named argument without value returns {..., <name>: ""}
        assertEquals(Map.of("CMD", "test",  "dummy", ""),
                     Parser.tokenize("test /dummy"));
        assertEquals(Map.of("CMD", "test",  "dummy", "", "another", ""),
    ...
    public void parse_validCommand_success() {
        ...
        // deadline
        assertEquals(new DeadlineCommand(Map.of("CMD", "deadline",
                                                "", "test description",
                                                "by", "tmr")),
                     parser.parse("deadline test description /by tmr"));

        // delete
        assertEquals(new DeleteCommand(Map.of("CMD", "delete",
                                              "", "5")),
                     parser.parse("delete 5"));

        // event
        assertEquals(new EventCommand(Map.of("CMD", "event",
                                             "", "test description",
                                             "from", "now",
                                             "to", "tmr")),
        ...
        // mark
        assertEquals(new MarkCommand(Map.of("CMD", "mark",
                                            "", "5")),
                     parser.parse("mark 5"));
        ...
        // todo
        assertEquals(new TodoCommand(Map.of("CMD", "todo",
                                              "", "test description")),
                     parser.parse("todo test description"));

        // unmark
        assertEquals(new UnmarkCommand(Map.of("CMD", "unmark",
                                            "", "5")),
