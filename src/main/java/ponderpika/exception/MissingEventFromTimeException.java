...
/**
 * Exception thrown when the timing information (only "From" time) is missing for an event task.
 * <p>
 * This class extends PonderPikaException to handle specific cases
 * where event timing details are required but not provided.
 * </p>
 */
public class MissingEventFromTimeException extends PonderPikaException {
     ...
     * stating that the "From" time for the event task is missing.
     *
     * @return A string that represents the exception
     */
    @Override
    public String toString() {
        return (super.toString() + "Missing \"From\" time for Event task!");
    }
}
