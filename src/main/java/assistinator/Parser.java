...
public class Parser {
     ...
     * @throws AssitinatorException If the input format is invalid or the index cannot be parsed.
     */
    public int parseIndex(String input) throws AssitinatorException {
        try {
            return Integer.parseInt(input.split(" ")[1]) - 1;
        } catch (ArrayIndexOutOfBoundsException | NumberFormatException e) {
            throw new AssitinatorException("Please provide a valid task number");
        }
    }
     ...
     * @throws AssitinatorException If the task type is invalid.
     */
    public Task parseTask(Command type, String input) throws AssitinatorException {
        switch (type) {
        case TODO:
        ...
        default:
            ...
            throw new AssitinatorException("Invalid task type");
        }
    }

    private Todo parseTodo(String input) throws AssitinatorException {
        String description = input.substring(input.indexOf(' ') + 1);
        if (description.equalsIgnoreCase("todo")) {
            throw new AssitinatorException("Please follow format: todo {task description}");
        }
        return new Todo(description);
    }

    private Deadline parseDeadline(String input) throws AssitinatorException {
        String[] parts = input.split(" /");
        if (parts.length != DEADLINE_INPUT_LENGTH || parts[1].equals("by")) {
            throw new AssitinatorException("Please follow format: deadline {task description} /by {deadline}");
        }
    ...
    private Event parseEvent(String input) throws AssitinatorException {
        String[] parts = input.split(" /");
        if (parts.length != EVENT_INPUT_LENGTH || parts[1].equals("from") || parts[2].equals("to")) {
            throw new AssitinatorException("Please follow format: event {task description} /from {start} /to {end}");
        }
        String description = parts[0].substring(parts[0].indexOf(' ') + 1);
