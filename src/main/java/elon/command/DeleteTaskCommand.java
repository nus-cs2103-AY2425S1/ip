...
public class DeleteTaskCommand extends Command {
  ...
  /**
   * Executes the delete task command by removing the task at the specified index from the task
   * list, saving the updated list, and returning a message from the Ui. If the index is invalid, it
   * will return an error message.
   *
   * @param list the task list from which the task will be deleted
   * @param ui the user interface to interact with the user
   * @param storage the storage to save the updated task list
   * @return a string representing the result of deleting the task or an error message if the index
   *     is invalid
