        ...
        while (1 == 1) {
            ...
            if (tkns.length > 1 ) {
                ...
                if (cmd.equals(CMD_todo)) {
                    if (param.isBlank()) {
                        say("Please provide a description for the todo.");
                        continue;
                    }
                ...
                if (cmd.equals(CMD_deadline)) {
                    if (param.isEmpty() || !param.contains("/by")) {
                        say("Please provide a description and deadline.",
                                "Usage:", "deadline return book /by Sunday");
                        continue;
                    }
                    String[] parts = param.split("/by", 2);
                    if (parts.length != 2 || parts[0].isBlank() || parts[1].isBlank()) {
                        say("Description and deadline must not be blank.",
                                "Usage:", "deadline return book /by Sunday");
                        continue;
                    }
                ...
                if (cmd.equals(CMD_event)) {
                    if (param.isEmpty() || !param.contains("/from") || !param.contains("/to")) {
                        say("Please provide a description, start date, and end date.",
                                "Usage:", "event project meeting /from Mon 2pm /to 4pm");
                        continue;
                    }
                    String[] parts = param.split("/from|/to", 3);
                    if (parts.length != 3 || parts[0].isBlank() || parts[1].isBlank() || parts[2].isBlank()) {
                        say("Description start date, and end date must not be blank.",
                                "Usage:", "event project meeting /from Mon 2pm /to 4pm");
                        continue;
                    }
